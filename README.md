...
- Authentication handled with Flask-Login and extras (2FA and password-less login) with Twilio/SMS
...
- The core module contains applications base template (including assets like CSS JS, and images).
...
3. Once your local PostgreSQL database is set up and running, run `flask db init` to set up the migrations folder. Then run `flask db migrate -m "Initial setup"` and `flask db upgrade`.
